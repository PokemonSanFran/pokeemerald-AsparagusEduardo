...
static void CB2_ReadHeldMail(void)
{
    ReadMail(&gSaveBlock2Ptr->mail[GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_MAIL)], CB2_ReturnToPartyMenuFromReadingMail, TRUE);
}
    ...
    else
    {
        ...
        else if (sFieldMoveCursorCallbacks[fieldMove].fieldMoveFunc() == TRUE)
        {
            ...
            case FIELD_MOVE_TELEPORT:
                mapHeader = Overworld_GetMapHeaderByGroupAndId(gSaveBlock2Ptr->lastHealLocation.mapGroup, gSaveBlock2Ptr->lastHealLocation.mapNum);
...
static void TryGiveMailToSelectedMon(u8 taskId)
{
    ...
    gPartyMenuUseExitCallback = FALSE;
    mail = &gSaveBlock2Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE + gPlayerPCItemPageInfo.cursorPos];
    if (GetMonData(mon, MON_DATA_HELD_ITEM) != ITEM_NONE)
    {
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_IS_EGG)
        ...
        || (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
            && gSaveBlock2Ptr->location.mapNum == MAP_NUM(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
