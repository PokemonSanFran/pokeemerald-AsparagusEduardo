    ...
    if (!gPaletteFade.active && MenuHelpers_ShouldWaitForLinkRecv() != TRUE)
    {
        ...
        case SELECT_BUTTON: // Quick Swap
             ...
             && CalculatePlayerPartyCount() >= 2)
            {
                ...
                else //If already in switch action, do the switch
                    HandleChooseMonSelection(taskId, slotPtr);
            }
            break;
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (FlagGet(FLAG_BADGE06_GET) && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES2), MOVE_FLY))
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, FIELD_MOVE_FLY + MENU_FIELD_MOVES);
    }

    if (FlagGet(FLAG_BADGE02_GET) && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES2), MOVE_FLASH))
    {
...
void Task_AbilityCapsule(u8 taskId)
{
    ...
    case 0:
        ...
        if (GetAbilityBySpecies(tSpecies, 0) == GetAbilityBySpecies(tSpecies, 1)
            || GetAbilityBySpecies(tSpecies, 1) == 0
            || tAbilityNum > 1
            || !tSpecies)
        {
            gPartyMenuUseExitCallback = FALSE;
