...
static void Mailbox_CompactMailList(void)
{
    ...
    for (i = PARTY_SIZE; i < MAIL_COUNT - 1; i++)
    {
        ...
        {
            if (gSaveBlock2Ptr->mail[i].itemId == ITEM_NONE)
                SWAP(gSaveBlock2Ptr->mail[i], gSaveBlock2Ptr->mail[j], temp);
        }
    }
}
...
{
    StringCopy(gStringVar1, gSaveBlock2Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE + gPlayerPCItemPageInfo.cursorPos].playerName);
...
static void Mailbox_FadeAndReadMail(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        MailboxMenu_Free();
        CleanupOverworldWindowsAndTilemaps();
        ReadMail(&gSaveBlock2Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE + gPlayerPCItemPageInfo.cursorPos], Mailbox_ReturnToFieldFromReadMail, TRUE);
        DestroyTask(taskId);
    }
}
...
{
    struct Mail *mail = &gSaveBlock2Ptr->mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE + gPlayerPCItemPageInfo.cursorPos];
