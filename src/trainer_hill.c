...
void ResetTrainerHillResults(void)
{
    ...
    gSaveBlock2Ptr->trainerHill.bestTime = 0;
    for (i = 0; i < NUM_TRAINER_HILL_MODES; i++)
        SetTimerValue(&gSaveBlock2Ptr->trainerHillTimes[i], HILL_MAX_TIME);
}
...
void InitTrainerHillBattleStruct(void)
{
    ...
    SetTrainerHillVBlankCounter(&gSaveBlock2Ptr->trainerHill.timer);
    FreeDataStruct();
}
...
static void SetUpDataStruct(void)
{
    if (sHillData == NULL)
    {
        ...
        CpuCopy32(sChallengeData[gSaveBlock2Ptr->trainerHill.mode], &sHillData->challenge, sizeof(sHillData->challenge) + sizeof(sHillData->floors));
        TrainerHillDummy();
    }
}
...
static void TrainerHillStartChallenge(void)
{
    ...
    if (!ReadTrainerHillAndValidate())
        gSaveBlock2Ptr->trainerHill.field_3D6E_0f = 1;
    else
        gSaveBlock2Ptr->trainerHill.field_3D6E_0f = 0;

    gSaveBlock2Ptr->trainerHill.unk_3D6C = 0;
    SetTrainerHillVBlankCounter(&gSaveBlock2Ptr->trainerHill.timer);
    gSaveBlock2Ptr->trainerHill.timer = 0;
    gSaveBlock2Ptr->trainerHill.spokeToOwner = 0;
    gSaveBlock2Ptr->trainerHill.checkedFinalTime = 0;
    gSaveBlock2Ptr->trainerHill.maybeECardScanDuringChallenge = 0;
    gSaveBlock1Ptr->frontier.trainerFlags = 0;
    gBattleOutcome = 0;
    gSaveBlock2Ptr->trainerHill.receivedPrize = 0;
}
...
static void GetOwnerState(void)
{
    ...
    if (gSaveBlock2Ptr->trainerHill.spokeToOwner)
        gSpecialVar_Result++;
    if (gSaveBlock2Ptr->trainerHill.receivedPrize && gSaveBlock2Ptr->trainerHill.checkedFinalTime)
        gSpecialVar_Result++;

    gSaveBlock2Ptr->trainerHill.spokeToOwner = TRUE;
}
...
static void GiveChallengePrize(void)
{
    ...
    if (sHillData->challenge.numFloors != NUM_TRAINER_HILL_FLOORS || gSaveBlock2Ptr->trainerHill.receivedPrize)
    {
    ...
    else if (AddBagItem(itemId, 1) == TRUE)
    {
        ...
        gSaveBlock2Ptr->trainerHill.receivedPrize = TRUE;
        gSaveBlock1Ptr->frontier.unk_EF9 = 0;
        gSpecialVar_Result = 0;
    }
    else
...
static void CheckFinalTime(void)
{
    if (gSaveBlock2Ptr->trainerHill.checkedFinalTime)
    {
        gSpecialVar_Result = 2;
    }
    else if (GetTimerValue(&gSaveBlock2Ptr->trainerHill.bestTime) > gSaveBlock2Ptr->trainerHill.timer)
    {
        SetTimerValue(&gSaveBlock2Ptr->trainerHill.bestTime, gSaveBlock2Ptr->trainerHill.timer);
        gSaveBlock2Ptr->trainerHillTimes[gSaveBlock2Ptr->trainerHill.mode] = gSaveBlock2Ptr->trainerHill.bestTime;
        gSpecialVar_Result = 0;
    }
    else
    {
    ...
    gSaveBlock2Ptr->trainerHill.checkedFinalTime = TRUE;
}
...
static void TrainerHillResumeTimer(void)
{
    if (!gSaveBlock2Ptr->trainerHill.spokeToOwner)
    {
        if (gSaveBlock2Ptr->trainerHill.timer >= HILL_MAX_TIME)
            gSaveBlock2Ptr->trainerHill.timer = HILL_MAX_TIME;
        else
            SetTrainerHillVBlankCounter(&gSaveBlock2Ptr->trainerHill.timer);
    }
}
...
static void TrainerHillSetPlayerLost(void)
{
    gSaveBlock2Ptr->trainerHill.hasLost = TRUE;
}

static void TrainerHillGetChallengeStatus(void)
{
    if (gSaveBlock2Ptr->trainerHill.hasLost)
    {
        // The player lost their last match.
        gSaveBlock2Ptr->trainerHill.hasLost = FALSE;
        gSpecialVar_Result = TRAINER_HILL_PLAYER_STATUS_LOST;
    }
    else if (gSaveBlock2Ptr->trainerHill.maybeECardScanDuringChallenge)
    {
...
static void BufferChallengeTime(void)
{
    ...
    total = gSaveBlock2Ptr->trainerHill.timer;
...
bool8 InTrainerHillChallenge(void)
{
    if (VarGet(VAR_TRAINER_HILL_IS_ACTIVE) == 0)
        return FALSE;
    else if (gSaveBlock2Ptr->trainerHill.spokeToOwner)
        return FALSE;
...
void PrintOnTrainerHillRecordsWindow(void)
{
    ...
    for (i = 0; i < NUM_TRAINER_HILL_MODES; i++)
    {
        ...
        y += 15;
        total = GetTimerValue(&gSaveBlock2Ptr->trainerHillTimes[i]);
        minutes = total / (60 * 60);
        total %= (60 * 60);
        ConvertIntToDecimalStringN(gStringVar1, minutes, STR_CONV_MODE_RIGHT_ALIGN, 2);
        secondsWhole = total / 60;
...
{
    u8 i, floorId;
    struct ObjectEventTemplate *eventTemplates = gSaveBlock2Ptr->objectEventTemplates;
...
static void TrainerHillSetMode(void)
{
    gSaveBlock2Ptr->trainerHill.mode = gSpecialVar_0x8005;
    gSaveBlock2Ptr->trainerHill.bestTime = gSaveBlock2Ptr->trainerHillTimes[gSpecialVar_0x8005];
}
...
static u16 GetPrizeItemId(void)
{
    ...
    if (gSaveBlock2Ptr->trainerHill.mode == HILL_MODE_EXPERT)
    ...
    minutes = (signed)(gSaveBlock2Ptr->trainerHill.timer) / (60 * 60);
    if (minutes < 12)
