...
void SpawnLinkPartnerObjectEvent(void)
{
    ...
    case DIR_WEST:
        j = 2;
        x = gSaveBlock2Ptr->pos.x - 1;
        y = gSaveBlock2Ptr->pos.y;
        break;
    case DIR_NORTH:
        j = 1;
        x = gSaveBlock2Ptr->pos.x;
        y = gSaveBlock2Ptr->pos.y - 1;
        break;
    case DIR_EAST:
        x = gSaveBlock2Ptr->pos.x + 1;
        y = gSaveBlock2Ptr->pos.y;
...
static void LoadLinkPartnerObjectEventSpritePalette(u8 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    {
        u8 obj = GetObjectEventIdByLocalIdAndMap(localEventId, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
...
{
    gSpecialVar_0x8004 = gSaveBlock2Ptr->pos.x;
    gSpecialVar_0x8005 = gSaveBlock2Ptr->pos.y;
}
...
static void PCTurnOnEffect_1(s16 isPcTurnedOn, s8 dx, s8 dy)
{
    ...
    MapGridSetMetatileIdAt(gSaveBlock2Ptr->pos.x + dx + MAP_OFFSET, gSaveBlock2Ptr->pos.y + dy + MAP_OFFSET, tileId | MAPGRID_COLLISION_MASK);
}
...
void SpawnCameraObject(void)
{
    u8 obj = SpawnSpecialObjectEventParameterized(OBJ_EVENT_GFX_BOY_1,
                                                  MOVEMENT_TYPE_FACE_DOWN,
                                                  OBJ_EVENT_ID_CAMERA,
                                                  gSaveBlock2Ptr->pos.x + MAP_OFFSET,
                                                  gSaveBlock2Ptr->pos.y + MAP_OFFSET,
                                                  3);
...
void RemoveCameraObject(void)
{
    ...
    RemoveObjectEventByLocalIdAndMap(OBJ_EVENT_ID_CAMERA, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
}
...
u8 TryUpdateRusturfTunnelState(void)
{
    if (!FlagGet(FLAG_RUSTURF_TUNNEL_OPENED)
        && gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(RUSTURF_TUNNEL)
        && gSaveBlock2Ptr->location.mapNum == MAP_NUM(RUSTURF_TUNNEL))
    {
...
{
    if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM)
        && gSaveBlock2Ptr->location.mapNum == MAP_NUM(BATTLE_FRONTIER_BATTLE_TOWER_MULTI_PARTNER_ROOM) &&
...
{
    u8 deptStoreFloor;
    switch (gSaveBlock2Ptr->dynamicWarp.mapNum)
    {
...
u16 GetDeptStoreDefaultFloorChoice(void)
{
    ...
    if (gSaveBlock2Ptr->dynamicWarp.mapGroup == MAP_GROUP(LILYCOVE_CITY_DEPARTMENT_STORE_1F))
    {
        switch (gSaveBlock2Ptr->dynamicWarp.mapNum)
        {
        case MAP_NUM(LILYCOVE_CITY_DEPARTMENT_STORE_5F):
...
static void ChangeDeoxysRockLevel(u8 rockLevel)
{
    ...
    TryGetObjectEventIdByLocalIdAndMap(LOCALID_BIRTH_ISLAND_EXTERIOR_ROCK, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup, &objectEventId);
...
void IncrementBirthIslandRockStepCount(void)
{
    ...
    if (gSaveBlock2Ptr->location.mapNum == MAP_NUM(BIRTH_ISLAND_EXTERIOR) && gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(BIRTH_ISLAND_EXTERIOR))
    {
        var++;
        if (var > 99)
            VarSet(VAR_DEOXYS_ROCK_STEP_COUNT, 0);
...
bool8 AbnormalWeatherHasExpired(void)
{
    ...
    {
        VarSet(VAR_ABNORMAL_WEATHER_STEP_COUNTER, 0);
        if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(UNDERWATER_MARINE_CAVE))
        {
            switch (gSaveBlock2Ptr->location.mapNum)
            {
        ...
        if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(UNDERWATER_ROUTE127))
        {
            switch (gSaveBlock2Ptr->location.mapNum)
            {
        ...
        if (gSaveBlock2Ptr->location.mapNum == sAbnormalWeatherMapNumbers[abnormalWeather - 1] &&
            gSaveBlock2Ptr->location.mapGroup == 0)
        {
            return TRUE;
        }
        else
...
u32 GetMartEmployeeObjectEventId(void)
{
    ...
    {
        if (gSaveBlock2Ptr->location.mapGroup == sPokeMarts[i][0])
        {
            if (gSaveBlock2Ptr->location.mapNum == sPokeMarts[i][1])
...
static void Task_CloseBattlePikeCurtain(u8 taskId)
{
    ...
    if (tFrameTimer[tCurrentFrame] == 0)
    {
        for (y = 0; y < CURTAIN_HEIGHT; y++)
        {
            ...
            {
                MapGridSetMetatileIdAt(gSaveBlock2Ptr->pos.x + x + MAP_OFFSET - 1,
                                       gSaveBlock2Ptr->pos.y + y + MAP_OFFSET - 3,
...
{
    if (gSaveBlock2Ptr->lastHealLocation.mapGroup == MAP_GROUP(DEWFORD_TOWN) && gSaveBlock2Ptr->lastHealLocation.mapNum == MAP_NUM(DEWFORD_TOWN))
...
#define FANCLUB_BITFIELD (gSaveBlock2Ptr->vars[VAR_FANCLUB_FAN_COUNTER - VARS_START])
...
{
    gSaveBlock2Ptr->vars[VAR_FANCLUB_FAN_COUNTER - VARS_START] = 0;
    gSaveBlock2Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] = 0;
}
...
void TryLoseFansFromPlayTimeAfterLinkBattle(void)
{
    if (DidPlayerGetFirstFans())
    {
        ...
        gSaveBlock2Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] = gSaveBlock1Ptr->playTimeHours;
    }
}
...
void TryLoseFansFromPlayTime(void)
{
    ...
    if (gSaveBlock1Ptr->playTimeHours < 999)
    {
        while (TRUE)
        {
            ...
            else if (gSaveBlock1Ptr->playTimeHours - gSaveBlock2Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] < 12)
            {
                return;
            }
            PlayerLoseRandomTrainerFan();
            gSaveBlock2Ptr->vars[VAR_FANCLUB_LOSE_FAN_TIMER - VARS_START] += 12;
            i++;
        }
    }
}
...
void BufferFanClubTrainerName(void)
{
    ...
    BufferFanClubTrainerName_(&gSaveBlock2Ptr->linkBattleRecords, whichLinkTrainer, whichNPCTrainer);
}
