...
static bool8 LoadBattlerSpriteGfx(u8 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            DecompressTrainerBackPic(gSaveBlock1Ptr->playerGender, battler);
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock1Ptr->playerGender, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
