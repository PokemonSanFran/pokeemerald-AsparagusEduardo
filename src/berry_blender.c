...
static const u8 sOpponentBerrySets[NUM_NPC_BERRIES * 2][3] =
{
    ...
    {BERRY_ASPEAR, BERRY_RAWST,  BERRY_PECHA},   // player chose Cheri Berry
    {BERRY_CHERI,  BERRY_ASPEAR, BERRY_RAWST},   // player chose Chesto Berry
    {BERRY_CHESTO, BERRY_CHERI,  BERRY_ASPEAR},  // player chose Pecha Berry
    {BERRY_PECHA,  BERRY_CHESTO, BERRY_CHERI},   // player chose Rawst Berry
    {BERRY_RAWST,  BERRY_PECHA,  BERRY_CHESTO},  // player chose Aspear Berry

    // These sets are used if the player chose a different berry (set is selected by player's berry % 5)
    {BERRY_CHERI,  BERRY_PECHA,  BERRY_RAWST},   // player chose Leppa, Figy, ...
    {BERRY_CHESTO, BERRY_RAWST,  BERRY_ASPEAR},  // player chose Oran, Wiki, ...
    {BERRY_PECHA,  BERRY_ASPEAR, BERRY_CHERI},   // player chose Persim, Mago, ...
    {BERRY_RAWST,  BERRY_CHERI,  BERRY_CHESTO},  // player chose Lum, Aguav, ...
    {BERRY_ASPEAR, BERRY_CHESTO, BERRY_PECHA},   // player chose Sitrus, Iapapa, ...
};
...
{
    u8 spriteId = CreateSpinningBerrySprite(ItemId_GetSecondaryId(itemId) - 1, 0, 80, playerId & 1);
...
{
    const struct Berry *berryInfo = GetBerryInfo(ItemId_GetSecondaryId(itemId));
...
static void SetOpponentsBerryData(u16 playerBerryItemId, u8 playersNum, struct BlenderBerry* playerBerry)
{
    ...
    u16 berryMasterDiff;
    u16 i;
    u8 playerBerryId = ItemId_GetSecondaryId(playerBerryItemId);
    ...
    for (i = 0; i < playersNum - 1; i++)
    {
        ...
        berryMasterDiff = playerBerryId - ItemId_GetSecondaryId(ITEM_SPELON_BERRY);
        ...
        SetPlayerBerryData(i + 1, gBerries[opponentBerryId].itemId);
    }
}
    ...
    case 5:
        ...
        for (i = 0; i < BLENDER_MAX_PLAYERS; i++)
        {
            if (sBerryBlender->chosenItemId[i] != 0)
                berryIds[i] = ItemId_GetSecondaryId(sBerryBlender->chosenItemId[i]);
            if (sBerryBlender->arrowIdToPlayerId[i] != NO_PLAYER)
            {
                PutWindowTilemap(i);
                CopyWindowToVram(i, COPYWIN_FULL);
