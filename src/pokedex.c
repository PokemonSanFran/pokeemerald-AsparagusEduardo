...
void ResetPokedex(void)
{
    ...
    gSaveBlock1Ptr->pokedex.unknown3 = 0;
    DisableNationalPokedex();
    for (i = 0; i < NUM_DEX_FLAG_BYTES; i++)
    {
        gSaveBlock1Ptr->pokedex.owned[i] = 0;
        gSaveBlock1Ptr->pokedex.seen[i] = 0;
        gSaveBlock2Ptr->seen1[i] = 0;
        gSaveBlock2Ptr->seen2[i] = 0;
    }
}
...
static void Task_HandleInfoScreenInput(u8 taskId)
{
    ...
    if ((JOY_NEW(DPAD_LEFT)
     || (JOY_NEW(L_BUTTON) && gSaveBlock1Ptr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR))
    ...
    if ((JOY_NEW(DPAD_RIGHT)
     || (JOY_NEW(R_BUTTON) && gSaveBlock1Ptr->optionsButtonMode == OPTIONS_BUTTON_MODE_LR))
...
s8 GetSetPokedexFlag(u16 nationalDexNo, u8 caseID)
{
    ...
    case FLAG_GET_SEEN:
        ...
        {
            if ((gSaveBlock1Ptr->pokedex.seen[index] & mask) == (gSaveBlock2Ptr->seen1[index] & mask)
             && (gSaveBlock1Ptr->pokedex.seen[index] & mask) == (gSaveBlock2Ptr->seen2[index] & mask))
                retVal = 1;
            else
            {
                gSaveBlock1Ptr->pokedex.seen[index] &= ~mask;
                gSaveBlock2Ptr->seen1[index] &= ~mask;
                gSaveBlock2Ptr->seen2[index] &= ~mask;
                retVal = 0;
            }
        }
        break;
    case FLAG_GET_CAUGHT:
        if (gSaveBlock1Ptr->pokedex.owned[index] & mask)
        {
            if ((gSaveBlock1Ptr->pokedex.owned[index] & mask) == (gSaveBlock1Ptr->pokedex.seen[index] & mask)
             && (gSaveBlock1Ptr->pokedex.owned[index] & mask) == (gSaveBlock2Ptr->seen1[index] & mask)
             && (gSaveBlock1Ptr->pokedex.owned[index] & mask) == (gSaveBlock2Ptr->seen2[index] & mask))
                retVal = 1;
            else
            {
