...
#define MOVES_COUNT_TOTAL (EGG_MOVES_ARRAY_COUNT + MAX_LEVEL_UP_MOVES + NUM_TECHNICAL_MACHINES + NUM_HIDDEN_MACHINES + 40)
...
static void PrintStatsScreen_Moves_Top(u8 taskId)
{
    ...
    else if (selected < (numEggMoves + numLevelUpMoves + numTMHMMoves))
    {
        ...
        if (ITEM_HM08 - item + 1 > NUM_HIDDEN_MACHINES)
        {
            TMHMString = sText_TM;
            TMNumber = item - ITEM_TM01 + 1;
        }
        else
        {
            TMHMString = sText_HM;
            TMNumber = item - ITEM_HM01 + 1;
        }
