...
{
    u8 i;
    struct ObjectEventTemplate *objectEvents = gSaveBlock2Ptr->objectEventTemplates;
    u16 localId = 0;
    ...
    for (i = 0; i < OBJECT_EVENT_TEMPLATES_COUNT; i++)
    {
        ...
        if (puzzleTileNum < 4)
        {
            ...
            objectEvents[i].x += x;
            objectEvents[i].y += y;
            if (GetObjectEventIdByLocalIdAndMap(objectEvents[i].localId, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup) != OBJECT_EVENTS_COUNT)
            {
...
void TurnRotatingTileObjects(void)
{
    ...
    objectEvents = gSaveBlock2Ptr->objectEventTemplates;
    for (i = 0; i < sRotatingTilePuzzle->numObjects; i++)
    {
        ...
        objectEventId = GetObjectEventIdByLocalIdAndMap(objectEvents[sRotatingTilePuzzle->objects[i].eventTemplateId].localId, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
