...
static void HandleSpecialTrainerBattleEnd(void)
{
    ...
    case SPECIAL_BATTLE_SECRET_BASE:
        ...
        {
            u16 itemBefore = GetMonData(&gSaveBlock2Ptr->playerParty[i], MON_DATA_HELD_ITEM);
...
void DoSpecialTrainerBattle(void)
{
    ...
    case SPECIAL_BATTLE_SECRET_BASE:
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            SetMonData(&gSaveBlock2Ptr->playerParty[i], MON_DATA_HELD_ITEM, &itemBefore);
        }
        CreateTask(Task_StartBattleAfterTransition, 1);
        PlayMapChosenOrBattleBGM(0);
...
static void SaveBattleTowerRecord(void)
{
    ...
    for (i = 0; i < EASY_CHAT_BATTLE_WORDS_COUNT; i++)
    {
        ...
        playerRecord->speechWon[i] = gSaveBlock2Ptr->easyChatBattleWon[i];
        playerRecord->speechLost[i] = gSaveBlock2Ptr->easyChatBattleLost[i];
    }
...
static void LoadMultiPartnerCandidatesData(void)
{
    ...
    objEventTemplates = gSaveBlock2Ptr->objectEventTemplates;
    lvlMode = gSaveBlock1Ptr->frontier.lvlMode;
    battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
...
static void AwardBattleTowerRibbons(void)
{
    ...
    if (GetCurrentBattleTowerWinStreak(lvlMode, battleMode) > 55)
    {
        for (i = 0; i < monCount; i++)
        {
            ...
            ribbons[i].count = 0;
            if (!GetMonData(&gSaveBlock2Ptr->playerParty[partyIndex], ribbonType))
            {
                gSpecialVar_Result = TRUE;
                SetMonData(&gSaveBlock2Ptr->playerParty[partyIndex], ribbonType, &gSpecialVar_Result);
                ribbons[i].count = GetRibbonCount(&gSaveBlock2Ptr->playerParty[partyIndex]);
            }
        }
    }
    ...
    if (gSpecialVar_Result)
    {
        ...
        {
            TryPutSpotTheCutiesOnAir(&gSaveBlock2Ptr->playerParty[ribbons[0].partyIndex], ribbonType);
        }
    }
}
...
static void FillEReaderTrainerWithPlayerData(void)
{
    ...
    if (gSaveBlock1Ptr->playerGender != MALE)
    {
        ereaderTrainer->facilityClass = gTowerFemaleFacilityClasses[(gSaveBlock1Ptr->playerTrainerId[0] + gSaveBlock1Ptr->playerTrainerId[1]
                                                        + gSaveBlock1Ptr->playerTrainerId[2] + gSaveBlock1Ptr->playerTrainerId[3]) % ARRAY_COUNT(gTowerFemaleFacilityClasses)];
    }
    else
    {
        ereaderTrainer->facilityClass = gTowerMaleFacilityClasses[(gSaveBlock1Ptr->playerTrainerId[0] + gSaveBlock1Ptr->playerTrainerId[1]
                                                        + gSaveBlock1Ptr->playerTrainerId[2] + gSaveBlock1Ptr->playerTrainerId[3]) % ARRAY_COUNT(gTowerMaleFacilityClasses)];
    }

    CopyTrainerId(ereaderTrainer->trainerId, gSaveBlock1Ptr->playerTrainerId);
    StringCopy_PlayerName(ereaderTrainer->name, gSaveBlock1Ptr->playerName);
