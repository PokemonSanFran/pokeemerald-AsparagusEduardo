...
static void Task_SwitchPageInReplaceMove(u8);
static void Task_ConfirmHMCantForget(u8);
...
static void CreateHeldItemSprite(struct Pokemon *);
...
static void PlayMonCry(void)
{
    ...
    if (!summary->isEgg)
    {
        if (ShouldPlayNormalMonCry(&sMonSummaryScreen->currentMon) == TRUE)
            ...
            PlayCry_ByMode(summary->species, 0, CRY_MODE_WEAK);
    }
}
