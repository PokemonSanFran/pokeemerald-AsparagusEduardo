...
static void BuyMenuDrawGraphics(void)
{
    ...
    PrintMoneyAmountInMoneyBoxWithBorder(0, 1, 13, GetMoney(&gSaveBlock2Ptr->money));
    ScheduleBgCopyTilemapToVram(0);
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            if (!IsEnoughMoney(&gSaveBlock2Ptr->money, sShopData->totalCost))
            {
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    maxQuantity = GetMoney(&gSaveBlock2Ptr->money) / sShopData->totalCost;
...
static void BuyMenuSubtractMoney(u8 taskId)
{
    ...
    RemoveMoney(&gSaveBlock2Ptr->money, sShopData->totalCost);
    PlaySE(SE_SHOP);
    PrintMoneyAmountInMoneyBox(0, GetMoney(&gSaveBlock2Ptr->money), 0);
