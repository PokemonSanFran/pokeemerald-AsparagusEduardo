...
{
    u32 i;
    u8 *dest = (u8 *)&gSaveBlock2Ptr->enigmaBerry;

    for (i = 0; i < sizeof(gSaveBlock2Ptr->enigmaBerry); i++)
        dest[i] = src[i];
}
...
static u32 GetEnigmaBerryChecksum(struct EnigmaBerry *enigmaBerry)
{
    ...
    dest = (u8 *)enigmaBerry;
    checksum = 0;
    for (i = 0; i < sizeof(gSaveBlock2Ptr->enigmaBerry) - sizeof(gSaveBlock2Ptr->enigmaBerry.checksum); i++)
        checksum += dest[i];
...
{
    if (!gSaveBlock2Ptr->enigmaBerry.berry.stageDuration)
        return FALSE;
    if (!gSaveBlock2Ptr->enigmaBerry.berry.maxYield)
        return FALSE;
...
void ObjectEventInteractionGetBerryTreeData(void)
{
    ...
    num = gSaveBlock2Ptr->location.mapNum;
    group = gSaveBlock2Ptr->location.mapGroup;
...
void ObjectEventInteractionRemoveBerryTree(void)
{
    ...
    SetBerryTreeJustPicked(gSpecialVar_LastTalked, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
}
