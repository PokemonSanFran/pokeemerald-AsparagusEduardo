...
void SetPlayerSecretBase(void)
{
    ...
    for (i = 0; i < TRAINER_ID_LENGTH; i++)
        gSaveBlock2Ptr->secretBases[0].trainerId[i] = gSaveBlock1Ptr->playerTrainerId[i];

    VarSet(VAR_CURRENT_SECRET_BASE, 0);
    StringCopyN(gSaveBlock2Ptr->secretBases[0].trainerName, gSaveBlock1Ptr->playerName, GetNameLength(gSaveBlock1Ptr->playerName));
    gSaveBlock2Ptr->secretBases[0].gender = gSaveBlock1Ptr->playerGender;
...
static void Task_EnterNewlyCreatedSecretBase(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        s8 secretBaseGroup = SECRET_BASE_ID_TO_GROUP(sCurSecretBaseId);
        SetWarpDestination(
            gSaveBlock2Ptr->location.mapGroup,
            gSaveBlock2Ptr->location.mapNum,
            WARP_ID_NONE,
...
{
    if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(SECRET_BASE_RED_CAVE1)
     && (u8)gSaveBlock2Ptr->location.mapNum <= MAP_NUM(SECRET_BASE_SHRUB4))
        return TRUE;
    else
        return FALSE;
}
    ...
    for (i = 0; i < numDecorations; i++)
    {
        ...
        if (permission == DECORPERM_SPRITE)
        {
             ...
             || MetatileBehavior_HoldsLargeDecoration(metatileBehavior) == TRUE)
            {
                ...
                TrySpawnObjectEvent(gSpecialVar_Result, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
                TryMoveObjectEventToMapCoords(gSpecialVar_Result, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup, gSpecialVar_0x8006, gSpecialVar_0x8007);
                TryOverrideObjectEventTemplateCoords(gSpecialVar_Result, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
