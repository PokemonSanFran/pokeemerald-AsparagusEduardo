...
static bool8 MapHasSpecies(const struct WildPokemonHeader *info, u16 species)
{
    ...
    if (sPokedexAreaScreen->state == TIME_NIGHT && info->landMonsNightInfo != NULL)
    {
        if (MonListHasSpecies(info->landMonsNightInfo, species, LAND_WILD_COUNT))
            return TRUE;
    } 
    else
    {
...
// As a result it's reading out of bounds of the fishing encounters tables.
    ...
    if (MonListHasSpecies(info->rockSmashMonsInfo, species, ROCK_WILD_COUNT))
        return TRUE;
    if (MonListHasSpecies(info->hiddenMonsInfo, species, 3))
        return TRUE;
    return FALSE;
}
