...
static bool8 CheckFeebas(void)
{
    ...
    if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(ROUTE119)
     && gSaveBlock2Ptr->location.mapNum == MAP_NUM(ROUTE119))
    {
...
static u16 GetCurrentMapWildMonHeaderId(void)
{
    ...
    for (i = 0; ; i++)
    {
        ...
        if (gWildMonHeaders[i].mapGroup == gSaveBlock2Ptr->location.mapGroup &&
            gWildMonHeaders[i].mapNum == gSaveBlock2Ptr->location.mapNum)
        {
            if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(ALTERING_CAVE) &&
                gSaveBlock2Ptr->location.mapNum == MAP_NUM(ALTERING_CAVE))
            {
...
static bool8 SetUpMassOutbreakEncounter(u8 flags)
{
    ...
    if (flags & WILD_CHECK_REPEL && !IsWildLevelAllowedByRepel(gSaveBlock2Ptr->outbreakPokemonLevel))
        return FALSE;

    CreateWildMon(gSaveBlock2Ptr->outbreakPokemonSpecies, gSaveBlock2Ptr->outbreakPokemonLevel);
    for (i = 0; i < MAX_MON_MOVES; i++)
        SetMonMoveSlot(&gEnemyParty[0], gSaveBlock2Ptr->outbreakPokemonMoves[i], i);
...
static bool8 DoMassOutbreakEncounterTest(void)
{
    if (gSaveBlock2Ptr->outbreakPokemonSpecies != SPECIES_NONE
     && gSaveBlock2Ptr->location.mapNum == gSaveBlock2Ptr->outbreakLocationMapNum
     && gSaveBlock2Ptr->location.mapGroup == gSaveBlock2Ptr->outbreakLocationMapGroup)
    {
        if (Random() % 100 < gSaveBlock2Ptr->outbreakPokemonProbability)
            return TRUE;
    }
    return FALSE;
}
...
static bool8 DoWildEncounterRateTest(u32 encounterRate, bool8 ignoreAbility)
{
    ...
    if (!ignoreAbility && !GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG))
    {
        ...
        if (ability == ABILITY_STENCH && gMapHeader.mapLayoutId == LAYOUT_BATTLE_FRONTIER_BATTLE_PYRAMID_FLOOR)
            ...
            encounterRate *= 2;
        else if (ability == ABILITY_SAND_VEIL && gSaveBlock2Ptr->weather == WEATHER_SANDSTORM)
            encounterRate /= 2;
    }
...
{
    if (gSaveBlock2Ptr->location.mapGroup != MAP_GROUP(SOOTOPOLIS_CITY)
