...
int GetUnionRoomTradeMessageId(struct RfuGameCompatibilityData player, struct RfuGameCompatibilityData partner, u16 playerSpecies2, u16 partnerSpecies, u8 requestedType, u16 playerSpecies, u8 isEventLegal)
{
    ...
    else
    {
        if (GetTypeBySpecies(playerSpecies2, 1) != requestedType
         && GetTypeBySpecies(playerSpecies2, 2) != requestedType) //tx_difficulty_challenges
...
void CreateInGameTradePokemon(void)
{
    ...
    {
        struct Pokemon *pokemon = &gEnemyParty[0];
        CreateMon(pokemon, SPECIES_WURMPLE, 10, 32, FALSE, 0, OT_ID_PRESET, 0); // (After the trade this pokemon is set as "Seen and caught" in the players pokedex!)
