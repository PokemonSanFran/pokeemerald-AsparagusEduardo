...
u8 BattleSetup_GetTerrainId(void)
{
    ...
    if (gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(ROUTE113) && gSaveBlock2Ptr->location.mapNum == MAP_NUM(ROUTE113))
        return BATTLE_TERRAIN_SAND;
...
void SetMapVarsToTrainer(void)
{
    if (sTrainerObjectEventLocalId != 0)
    {
        ...
        gSelectedObjectEvent = GetObjectEventIdByLocalIdAndMap(sTrainerObjectEventLocalId, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
    }
}
...
static bool32 UpdateRandomTrainerRematches(const struct RematchTrainer *table, u16 mapGroup, u16 mapNum)
{
    ...
    for (i = 0; i <= REMATCH_SPECIAL_TRAINER_START; i++)
    {
        ...
        {
            if (gSaveBlock2Ptr->trainerRematches[i] != 0)
            {
...
void IncrementRematchStepCounter(void)
{
    ...
    {
        if (gSaveBlock2Ptr->trainerRematchStepCounter >= STEP_COUNTER_MAX)
            gSaveBlock2Ptr->trainerRematchStepCounter = STEP_COUNTER_MAX;
        else
            gSaveBlock2Ptr->trainerRematchStepCounter++;
    }
}

static bool32 IsRematchStepCounterMaxed(void)
{
    if (HasAtLeastFiveBadges() && gSaveBlock2Ptr->trainerRematchStepCounter >= STEP_COUNTER_MAX)
        return TRUE;
    else
        return FALSE;
}
