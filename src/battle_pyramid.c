...
static void SetPickupItem(void)
{
    ...
    u32 round = (gSaveBlock1Ptr->frontier.pyramidWinStreaks[lvlMode] / FRONTIER_STAGES_PER_CHALLENGE) % TOTAL_ROUNDS;
...
static void HidePyramidItem(void)
{
    struct ObjectEventTemplate *events = gSaveBlock2Ptr->objectEventTemplates;
    int i = 0;
...
static void RestorePyramidPlayerParty(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        {
            if (GetMonData(&gSaveBlock2Ptr->playerParty[partyIndex], MON_DATA_SPECIES, NULL) == GetMonData(&gPlayerParty[j], MON_DATA_SPECIES, NULL))
            {
                for (k = 0; k < MAX_MON_MOVES; k++)
                {
                    for (l = 0; l < MAX_MON_MOVES; l++)
                    {
                        if (GetMonData(&gSaveBlock2Ptr->playerParty[partyIndex], MON_DATA_MOVE1 + l, NULL) == GetMonData(&gPlayerParty[j], MON_DATA_MOVE1 + k, NULL))
                            break;
                    }
                    if (l == MAX_MON_MOVES)
                        SetMonMoveSlot(&gPlayerParty[j], MOVE_SKETCH, k);
                }
                gSaveBlock2Ptr->playerParty[partyIndex] = gPlayerParty[j];
...
static void MarkPyramidTrainerAsBattled(u16 trainerId)
{
    ...
    gSaveBlock2Ptr->objectEventTemplates[gSpecialVar_LastTalked - 1].movementType = MOVEMENT_TYPE_WANDER_AROUND;
...
void GenerateBattlePyramidWildMon(void)
{
    ...
    u16 round = (gSaveBlock1Ptr->frontier.pyramidWinStreaks[lvl] / FRONTIER_STAGES_PER_CHALLENGE) % TOTAL_ROUNDS;
        ...
        for (y = 0; y < mapLayout->height; y++)
        {
            for (x = 0; x < mapLayout->width; x++)
            {
                ...
                else if (i != exitSquareId)
                {
                    ...
                    {
                        gSaveBlock2Ptr->pos.x = (mapLayout->width * (i % PYRAMID_FLOOR_SQUARES_WIDE)) + x;
                        gSaveBlock2Ptr->pos.y = (mapLayout->height * (i / PYRAMID_FLOOR_SQUARES_WIDE)) + y;
                    }
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    struct ObjectEventTemplate *floorEvents = gSaveBlock2Ptr->objectEventTemplates;
