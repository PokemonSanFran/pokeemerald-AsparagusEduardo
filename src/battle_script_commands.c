    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                #if B_SPLIT_EXP < GEN_6
                    if (gSaveBlock2Ptr->expShare || holdEffect == HOLD_EFFECT_EXP_SHARE)
                        gBattleMoveDamage += gExpShareExp;
                #else
                    if (gSaveBlock2Ptr->expShare || (holdEffect == HOLD_EFFECT_EXP_SHARE && gBattleMoveDamage == 0))
                        gBattleMoveDamage += gExpShareExp;
                #endif
...
static void Cmd_transformdataexecution(void)
{
    ...
    else
    {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            if (gBattleMons[gBattlerAttacker].moves[i] >= MOVE_HIDDEN_POWER_GROUND && gBattleMons[gBattlerAttacker].moves[i] <= MOVE_HIDDEN_POWER_FLYING)
...
static void Cmd_mimicattackcopy(void)
{
    ...
    else
    {
        ...
        {
            gChosenMove = 0xFFFF;
            if (gLastMoves[gBattlerTarget] >= MOVE_HIDDEN_POWER_GROUND && gLastMoves[gBattlerTarget] <= MOVE_HIDDEN_POWER_FLYING)
...
static void Cmd_pickup(void)
{
    ...
    else
    {
        ...
        if (pickupSuccess == 1) // only one Pokemon has picked something up, print solo message
        {
            ...
            CopyItemName(item, gBattleTextBuff3);
            BattleScriptPush(gBattlescriptCurrInstr + 1);
