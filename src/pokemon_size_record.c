...
static u8 CompareMonSize(u16 species, u16 *sizeRecord)
{
    ...
    else
    {
        ...
        if (GetMonData(pkmn, MON_DATA_IS_DUCK) == TRUE || GetMonData(pkmn, MON_DATA_SPECIES) != species)
        {
            return 1;
        }
        else
...
void GiveGiftRibbonToParty(u8 index, u8 ribbonId)
{
    ...
    if (index < GIFT_RIBBONS_COUNT && ribbonId <= MAX_GIFT_RIBBON)
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if (GetMonData(mon, MON_DATA_SPECIES) != 0 && GetMonData(mon, MON_DATA_SANITY_IS_DUCK) == 0)
            {
