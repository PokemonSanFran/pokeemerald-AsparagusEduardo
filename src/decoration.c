...
{
    gTasks[taskId].tInitialX = gSaveBlock2Ptr->pos.x;
    gTasks[taskId].tInitialY = gSaveBlock2Ptr->pos.y;
...
{
    DrawWholeMapView();
    SetWarpDestination(gSaveBlock2Ptr->location.mapGroup, gSaveBlock2Ptr->location.mapNum, WARP_ID_NONE, gTasks[taskId].tInitialX, gTasks[taskId].tInitialY);
    WarpIntoMap();
}
...
void SetDecoration(void)
{
    ...
    for (i = 0; i < NUM_DECORATION_FLAGS; i++)
    {
        if (FlagGet(FLAG_DECORATION_1 + i) == TRUE)
        {
            ...
            TrySpawnObjectEvent(gSpecialVar_0x8005, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
            TryMoveObjectEventToMapCoords(gSpecialVar_0x8005, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup, gSpecialVar_0x8006, gSpecialVar_0x8007);
            TryOverrideObjectEventTemplateCoords(gSpecialVar_0x8005, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup);
            break;
        }
    }
}
...
static void SetDecorRearrangementFlagIdIfFlagUnset(void)
{
    ...
    {
        if (gSaveBlock2Ptr->objectEventTemplates[i].x == xOff && gSaveBlock2Ptr->objectEventTemplates[i].y == yOff && !FlagGet(gSaveBlock2Ptr->objectEventTemplates[i].flagId))
        {
            sDecorRearrangementDataBuffer[sCurDecorSelectedInRearrangement].flagId = gSaveBlock2Ptr->objectEventTemplates[i].flagId;
            break;
        }
    }
}
