...
{
    u8 objectEventId;
    TryGetObjectEventIdByLocalIdAndMap(LOCALID_FARAWAY_ISLAND_MEW, gSaveBlock2Ptr->location.mapNum, gSaveBlock2Ptr->location.mapGroup, &objectEventId);
...
void UpdateFarawayIslandStepCounter(void)
{
    ...
    if (gSaveBlock2Ptr->location.mapNum == MAP_NUM(FARAWAY_ISLAND_INTERIOR)
     && gSaveBlock2Ptr->location.mapGroup == MAP_GROUP(FARAWAY_ISLAND_INTERIOR))
    {
        steps++;
        if (steps >= 9999)
