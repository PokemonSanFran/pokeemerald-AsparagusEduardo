...
static void SaveSelectedParty(void)
{
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (monId < PARTY_SIZE)
            gSaveBlock2Ptr->playerParty[gSaveBlock1Ptr->frontier.selectedPartyMons[i] - 1] = gPlayerParty[i];
    }
}
...
static void RestoreHeldItems(void)
{
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        ...
        {
            u16 item = GetMonData(&gSaveBlock2Ptr->playerParty[gSaveBlock1Ptr->frontier.selectedPartyMons[i] - 1], MON_DATA_HELD_ITEM, NULL);
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        ...
        if (monId < PARTY_SIZE)
        {
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                ...
                {
                    if (GetMonData(&gSaveBlock2Ptr->playerParty[gSaveBlock1Ptr->frontier.selectedPartyMons[i] - 1], MON_DATA_MOVE1 + k, NULL)
...
static void Fill1PRecords(struct RankingHall1P *dst, s32 hallFacilityId, s32 lvlMode)
{
    ...
    for (i = 0; i < HALL_RECORDS_COUNT; i++)
        record1P[i] = gSaveBlock1Ptr->hallRecords1P[hallFacilityId][lvlMode][i];
...
static void Fill2PRecords(struct RankingHall2P *dst, s32 lvlMode)
{
    ...
    for (i = 0; i < HALL_RECORDS_COUNT; i++)
        record2P[i] = gSaveBlock1Ptr->hallRecords2P[lvlMode][i];
...
void ClearRankingHallRecords(void)
{
    ...
    for (i = 0; i < HALL_FACILITIES_COUNT; i++)
    {
        for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
        {
            ...
            {
                CopyTrainerId(gSaveBlock1Ptr->hallRecords1P[i][j][k].id, ZERO);
                gSaveBlock1Ptr->hallRecords1P[i][j][k].name[0] = EOS;
                gSaveBlock1Ptr->hallRecords1P[i][j][k].winStreak = 0;
            }
        }
    }
    ...
    for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
    {
        for (k = 0; k < HALL_RECORDS_COUNT; k++)
        {
            CopyTrainerId(gSaveBlock1Ptr->hallRecords2P[j][k].id1, ZERO);
            CopyTrainerId(gSaveBlock1Ptr->hallRecords2P[j][k].id2, ZERO);
            gSaveBlock1Ptr->hallRecords2P[j][k].name1[0] = EOS;
            gSaveBlock1Ptr->hallRecords2P[j][k].name2[0] = EOS;
            gSaveBlock1Ptr->hallRecords2P[j][k].winStreak = 0;
        }
    }
}
